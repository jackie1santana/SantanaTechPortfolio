...
async function load({
  ...
  if (configPath) {
    if (_path.default.extname(configPath) === '.js') {
      logger.warn({
    ...
    if (configFile) {
      ...
      let configFilePlugins = Array.isArray(configFile.plugins) ? configFile.plugins : Object.keys(configFile.plugins);

      for (let p of configFilePlugins) {
        // JavaScript configs can use an array of functions... opt out of all caching...
        if (typeof p === 'function') {
          configFile.__contains_functions = true; // This should enforce the config to be revalidated as it can contain functions and is JS
